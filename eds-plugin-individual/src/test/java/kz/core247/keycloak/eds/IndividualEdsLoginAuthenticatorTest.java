...
public class IndividualEdsLoginAuthenticatorTest {
    ...
    public void testActionValidCertificate() throws LibCertificateException {
        ...
        try (MockedStatic<CertificateParser> mock = Mockito.mockStatic(CertificateParser.class);
            MockedStatic<IndividualUserVerifierImpl> mockVerifier = Mockito.mockStatic(IndividualUserVerifierImpl.class)) {
            IndividualEdsLoginAuthenticator authenticator = new IndividualEdsLoginAuthenticator();
            AuthenticationFlowContext mockContext = Mockito.mock(AuthenticationFlowContext.class);
            ...
            mock.when(() -> CertificateParser.parseMetadata("valid certificate", true)).thenReturn(mockCertificate);
            mockVerifier.when(() -> IndividualUserVerifierImpl.verify(mockUser, mockCertificate)).thenReturn(true);
            // when
            authenticator.action(mockContext);
